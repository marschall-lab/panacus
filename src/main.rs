/* standard use */
use std::fs;
use std::io::prelude::*;
use std::time::Instant;

/* private use */
mod abacus;
mod cli;
mod graph;
mod io;

fn main() -> Result<(), std::io::Error> {
    env_logger::init();
    let timer = Instant::now();

    // print output to stdout
    let mut out = std::io::BufWriter::new(std::io::stdout());

    // read parameters and store them in memory
    let params = cli::read_params().unwrap();

    // preprocessing
    log::info!("preprocessing: processing nodes and counting P/W lines");
    let mut data = std::io::BufReader::new(fs::File::open(&params.gfa_file)?);
    let prep: abacus::Prep = io::preprocessing(&mut data, params.groups, params.subset_coords);
    log::info!(
        "..done; found {} paths/walks and {} nodes",
        prep.path_segments.len(),
        prep.node2id.len()
    );

    // creating the abacus from the gfa
    log::info!("loading graph from {}", params.gfa_file);
    let mut data = std::io::BufReader::new(fs::File::open(&params.gfa_file)?);
    let abacus = abacus::Abacus::from_gfa(&mut data, prep);
    log::info!(
        "abacus has {} path groups and {} countables",
        abacus.groups.len(),
        abacus.countable.len()
    );

    // constructing histogram
    log::info!("constructing histogram..");
    let hist = abacus.construct_hist();
    log::info!("done");

    //let mut tot = 0;
    //for i in 1..hist.len() {
    //    println!("{}", hist[i]);
    //    tot += hist[i];
    //}
    //log::info!("sum of hist: {}",tot);

    log::info!("constructing pangenome growth..");
    abacus::Abacus::hist2pangrowth(hist);
    log::info!("done");


    out.flush()?;
    let duration = timer.elapsed();
    log::info!("done; time elapsed: {:?} ", duration);

    Ok(())
}
